...
現在のレイアウト・表示方法は維持不要。以下のルールに従ってUIを刷新する。

### 1. ナビゲーション
- **PC:** 左サイドバーに固定ナビ。メイン3タブ（ユニット/キャラ/サポーター）は上部に大きく配置。ツール系（ダメ計算・チーム編成・ティア）はサイドバー下部にグルーピング
- **スマホ:** 画面下部にボトムナビバー（ネイティブアプリ風）。メイン3タブ + 「ツール」で4つに集約。ツールタップでサブメニュー展開

### 2. フィルタのプログレッシブ開示
- **第1層（常時表示）:** 検索バー + レアリティ
- **第2層（1タップ展開）:** タイプ・シリーズ・入手方法
- **第3層（「詳細フィルタ」ボタン）:** デバフ・地形・MAP兵器など
- アクティブフィルタはチップ表示、×で個別解除

### 3. カードの2モード化
- **コンパクトモード（デフォルト）:** 画像 + 名前 + レア + タイプ + 戦闘力のみ。1行4〜5枚。ホバーでステータスをツールチップ
- **詳細モード:** ステータスをバーグラフで視覚化。攻撃力・HP・防御力を横棒グラフ表示

### 4. 詳細画面
- モーダル→スライドインパネルに変更
- **PC:** リスト右半分がスプリットビューで詳細表示。カードクリックで右側更新
- **スマホ:** 画面下からスライドアップシート。半引き上げでサマリー、フルスワイプで全情報

### 5. ステータス比較
- カード長押し or チェックボックスで選択 → 画面下部にフローティング比較バー → タップでレーダーチャート・テーブル並列比較

### 6. テーブルビュー
- Notion/Airtable風インタラクティブテーブル
- 列の並べ替え・表示/非表示カスタマイズ・幅調整
- スマホ: 横スワイプで列スクロール、名前列は固定

### 7. ビジュアル
- レアリティカラーの洗練（UR=ゴールドグラデーション、EX=虹色ボーダーアニメ）
- ステータス数値にモノスペースフォント（桁揃え）
- フィルタ切替・タブ遷移にトランジション。カード出現にスタガーアニメーション

### 8. スマホ専用UX
- タブ切替を左右スワイプ対応
- カード左スワイプ→「比較に追加」、右スワイプ→「お気に入り」
- 検索バーはスクロールしても上部固定
